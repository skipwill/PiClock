...
def tick():
    ...
    global tidepixmap, tidepixmap2  # tide
    global lastmin, lastday, lasttimestr
    global clockrect
    global datex, datex2, datey2, pdy
    global sun, daytime, sunrise, sunset
    global bottom
    global pangle,typel, datel,datepl, prevday
    #print ("In Tick")
    nowtz = datetime.datetime.now(tz=tzlocal.get_localzone())
   # print ("now ",now)
...
### tide ############################################################################
   ...
   # prevday = datetime.datetime.now().day - 1
   # #print (prevday,datetime.datetime.now().day)
    if (prevday != datetime.datetime.now().day) or (prevday == 0):
         print ("In compare")
         datel = []
         typel = []
         datepl = []
         prevday = datetime.datetime.now().day
         begdated = datetime.datetime.now() - timedelta(days=1)
         enddated = datetime.datetime.now() + timedelta(days=1)
         begdate = begdated.strftime("%Y%m%d")
         enddate = enddated.strftime("%Y%m%d")
         #print ("Fetch new data", begdate, enddate)
         url = 'https://tidesandcurrents.noaa.gov/api/datagetter?product=predictions&application=NOS.COOPS.TAC.WL&begin_date=' + begdate + '&end_date=' + enddate + '&datum=MLLW&station=' + Config.tidestation + '&time_zone=lst_ldt&units=english&interval=hilo&format=json'
         #print ("Get tide data ",url)
         w = requests.get(url).json()
         #print(w)
         i = 1
         while i < 10:
              typel.append(w['predictions'][i]['type'])
              datel.append(w['predictions'][i]['t'])
              datepl.append(w['predictions'][i - 1]['t'])
              i += 1
   
            
    i = 0  # this was 0 but seemed like an error so i changed it to 1
    while i < 10:
          ...
          datett = datetime.datetime.strptime(datet,"%Y-%m-%d %H:%M")
          datetp = datepl[i]
          datettp = datetime.datetime.strptime(datetp,"%Y-%m-%d %H:%M")
         # nowy = datetime.datetime.strptime(nowx,"%Y-%m-%d %H:%M")
          i += 1
          difft = (datett - now)
          difftp = datett - datettp
       
          totmintotide = int(difftp.total_seconds() // 60)      
          mintotide = int(difft.total_seconds() // 60)
       
          if mintotide > 0:
                revmin = totmintotide - mintotide
                if type == "H":
                     pangle = round (180.0 /totmintotide ,2) * revmin + 180
                     # print datetime.datetime.now(), type, "totmin", totmintotide, "mintotide", mintotide, "revmin", revmin, "At",datett, "angle", angle
                     break
            
                else:
                     #print datetime.now(), "  ",mintotide, " minutes to the next Low tide at",datett
                     pangle = round (180.0 /totmintotide ,2) * revmin
                    # print (datetime.datetime.now(), type, "totmin", totmintotide, "mintotide", mintotide, "revmin", revmin, "At",datett, "angle", angle)
                     break
 ...
 #### tide #########################################################
    ...
    if Config.digital:
        ...
        if now.minute != lastmin:
            ...
            print (angle)
            ts = tidepixmap.size()
            tidepixmap2 = tidepixmap.transformed(
                QtGui.QTransform().scale(
                    float(clockrect.width()) / ts.height(),
                    float(clockrect.height()) / ts.height()
                ).rotate(angle),
            ...
            tidehand.setPixmap(tidepixmap2)
            ts = tidepixmap2.size()
            tidehand.setGeometry(
...
if not Config.digital:
    ...
    tidehand = QtWidgets.QLabel(frame1)
    tidehand.setObjectName("tidehand")
    tidehand.setStyleSheet("#tidehand { background-color: transparent; }")
    ...
    tidepixmap = QtGui.QPixmap(Config.tidehand)
    tidepixmap2 = QtGui.QPixmap(Config.tidehand)
else:
